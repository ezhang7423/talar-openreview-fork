...
class EvalCallback(EventCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            self.logger.record(f"eval/{self.name}/mean_reward", float(mean_reward))
            self.logger.record(f"eval/{self.name}/mean_ep_length", mean_ep_length)

            if len(self._is_success_buffer) > 0:
                success_rate = np.mean(self._is_success_buffer)
                if self.verbose >= 1:
                    print(f"Success rate: {100 * success_rate:.2f}%")
                self.logger.record(f"eval/{self.name}/success_rate", success_rate)
...
class MultiTaskEvalCallback(EvalCallback):
    def __init__(
        self,
        eval_env: Union[gym.Env, VecEnv],
        callback_on_new_best: Optional[BaseCallback] = None,
        ...
        self._task_is_success_buffer = {}
        super().__init__(eval_env, callback_on_new_best, callback_after_eval, n_eval_episodes, eval_freq, log_path, best_model_save_path, deterministic, render, verbose, warn, name)
    ...
    def _log_success_callback(self, locals_: Dict[str, Any], globals_: Dict[str, Any]) -> None:
        ...
        if locals_["done"]:
            maybe_is_success = info.get("is_success")
            eval_result_dict = info.get("eval_result_dict")
            if maybe_is_success is not None:
                self._is_success_buffer.append(maybe_is_success)
            if eval_result_dict is not None:
                task_name = eval_result_dict['task_name']
                task_is_success = eval_result_dict['is_success']
                if task_name in self._task_is_success_buffer:
                    self._task_is_success_buffer[task_name].append(task_is_success)
                else:
                    self._task_is_success_buffer[task_name] = [task_is_success]
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            if self.log_path is not None:
                ...
                if len(self._is_success_buffer) > 0:
                    self.evaluations_successes.append(self._is_success_buffer)
                    kwargs = dict(successes=self.evaluations_successes)
                    for task_name, is_success_list in self._task_is_success_buffer.items():
                        kwargs[task_name] = is_success_list
